...
class DeleteSpendingCategoriesTestCase(TestCase):
    ...
    def test_access_delete_not_default_categories_income_when_log_in_as_user(self):
        # Log in as the test user
        self.client.login(username=self.user.email, password='Password123')

        # Update the url with Income category
        category_1 = Categories.objects.get(name='Salary')
        url = reverse('delete_spending_categories', kwargs={'category_id': category_1.id})
        ...
        # Check that the category was deleted
        with self.assertRaises(Categories.DoesNotExist):
            Categories.objects.get(id=category_1.id)
    ...
    def test_access_delete_default_categories_expenditure_when_log_in_as_user(self):
        # Log in as the test user
        self.client.login(username=self.user.email, password='Password123')
    ...
    def test_access_delete_default_categories_income_when_log_in_as_user(self):
        # Log in as the test user
        self.client.login(username=self.user.email, password='Password123')
